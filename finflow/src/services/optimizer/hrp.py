...
from athena.src.core.services.optimizer import BaseOptimizer
...
class HRP(Optimizer):
    ...
    def __init__(
        self, historical_prices: pd.DataFrame, covariance_function: str, linkage_method: str
    ) -> None:
    ...
    def _get_covariance(self) -> pd.DataFrame:
        covariance = risk_matrix(self.historical_prices, method=self.covariance_function)
    ...
    def _optimize(self) -> HRPOpt:
        ...
        if len(covariance.columns) == 1:  # When there is only one asset
            hrp = pd.DataFrame(
                [1.0], columns=covariance.columns, index=[self.historical_prices.index[-1]]
            )
